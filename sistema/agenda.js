        ...
        financeiroEventsData.forEach(f => {
            ...
            if (dateStr) {
                calendar.addEvent({
                    ...
                    borderColor: color,
                    extendedProps: {
                        descricao: 'Evento do mÛdulo financeiro.',
...
function initCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            if (info.event.extendedProps && info.event.extendedProps.tipoEvento === 'FINANCEIRO') {
                if (typeof showToast === 'function') {
                    showToast('Conta a pagar/receber. Acesse o mÛdulo Financeiro para visualizar.', 'info');
                } else {
                    alert('Acesse o mÛdulo Financeiro para visualizar esta conta.');
                }
                return;
            }
        ...
        eventDrop: function(info) {
            if (info.event.extendedProps && info.event.extendedProps.tipoEvento === 'FINANCEIRO') {
                info.revert();
                if (typeof showToast === 'function') showToast('N„o È possÌvel reagendar contas por aqui.', 'error');
                return;
            }
...
function carregarEventos() {
    if (window.__paginaBloqueadaPorPermissao || (typeof window.verificarPermissaoRota === 'function' && !window.verificarPermissaoRota(window.location.pathname).permitido)) {
        console.warn('Bloqueando execu√ß√£o: usu√°rio sem permiss√£o para esta rota.');
        return;
    }
...
function abrirModalEvento(eventoObj = null, dataInicio = '', dataFim = '') {
    ...
    if (eventoObj) {
        ...
        // Se veio do click no calend√°rio (dataInicio ex: '2023-10-15')
        if (dataInicio) {
            // Se tiver T, tira
...
async function salvarEvento() {
    ...
    if (!titulo || !dataIniStr) {
        showToast("T√≠tulo e Data s√£o obrigat√≥rios.", "warning");
        return;
    }
