...
$htmlFiles = Get-ChildItem -Path "g:\site sistema" -Filter "*.html"
foreach ($f in $htmlFiles) {
    $html = [System.IO.File]::ReadAllText($f.FullName, $utf8)
    
    $startTag = '<!-- DETALHES DA VENDA/OR&#199;AMENTO -->'
    # Fallback if it has the moji or normal chars
    $idx1 = $html.IndexOf('<!-- DETALHES DA VENDA')
    $idx2 = $html.IndexOf('<!-- CONFIRMA', $idx1 + 10)
    
    if ($idx1 -ge 0 -and $idx2 -gt $idx1) {
        $prefix = $html.Substring(0, $idx1)
        # Find exactly where the next tag starts
        $suffixStart = $html.IndexOf('<!-- CONFIRMA', $idx1)
        if ($suffixStart -gt 0) {
            $suffix = $html.Substring($suffixStart)
            $newHtml = $prefix + $html_new_modal + "`r`n`r`n    " + $suffix
            [System.IO.File]::WriteAllText($f.FullName, $newHtml, $utf8)
        }
    }
}
...
Write-Host "Beautification completed!"
