...
function baixarPlanilhaModeloProduto() {
    ...
    const exemplo1 = "Mesa de Jantar Madeira Maci�a;78900000000;Mesas;500,00;750,00;10\n";
...
async function processarPlanilhaProdutos(event) {
    ...
    reader.onload = async function (e) {
        try {
            ...
            if (linhas.length <= 1) {
                showToast("A planilha parece estar vazia ou s� tem o cabe�alho.", "error");
                return;
            }
            ...
            // Leitura din�mica do cabe�alho para suportar planilha velha ou nova
            ...
            if (hCols[0].includes('nome')) {
                ...
                colIndex.preco = hCols.findIndex(c => c.includes('preco') || c.includes('pre�o') || c.includes('venda'));
            ...
            for (let i = 1; i < linhas.length; i++) {
                ...
                if (!existe) {
                    ...
                    if (estoque > 0) {
                        ...
                        batch.set(karRef, { data: new Date().toISOString(), ref: "Importa��o de Planilha", prodId: docRef.id, prodNome: nome, qtd: estoque, tipo: "INICIAL" });
                    }
                    produtosAdicionados++;
                }
            }
