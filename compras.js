// ==========================================
// GESTÃƒO.JS - ERP FINANCEIRO, DASHBOARD E PROJEÃƒâ€¡Ãƒâ€¢ES
// ==========================================
...
const categoriasPagar = ['Fornecedores / Compras', 'Impostos (DAS, ICMS, etc)', 'SalÃ¡rios / Folha', 'Aluguel', 'ÃƒÃƒÃgua', 'Energia', 'Internet / Telefonia', 'Contabilidade', 'Sistema / Software', 'IPTU', 'Outras Despesas'];
...
// ==========================================
// MIGRAÃƒâ€¡ÃƒO AUTOMÃƒÃƒÃTICA DO BANCO ANTIGO
// ==========================================
async function migrarDadosSeNecessario() {
    try {
        ...
        // ColeÃ§Ãµes novas estÃ£o vazias Ã¢Ã¢Ã¢â‚¬Å¡Â¬Ã¢Ã¢â€šÂ¬Ã tenta ler do banco antigo
...
function imprimirArea(areaId) {
    ...
    if(!element) return showToast("ÃƒÃƒÃrea de impressÃ£o nÃ£o encontrada.", "error");
...
function baixarPDF(areaId, filename) {
    const element = document.getElementById(areaId); 
    if(!element) return showToast("Erro: ÃƒÃƒÃrea do PDF nÃ£o encontrada.", "error");
...
// ==========================================
// 3. FINANCEIRO E CAIXA FÃƒÃƒâ€šÃSICO
// ==========================================
...
// ==========================================
// 5. MODAL DE CADASTRO/EDIÃƒâ€¡ÃƒO DE CONTA (COM RECORRÃƒÃƒâ€¦ NCIA)
// ==========================================
...
async function salvarCompraManual() {
    ...
    if(!apenasValor) {
        if(compraManualItens.length === 0) return showToast("Adicione itens vÃ¡lidos!", "error");
...
function exportarDadosParaIA() {
    ...
    let relatorioTexto = `=== RELATÃƒâ€œRIO FINANCEIRO E DE GESTÃƒO - FC MÃƒâ€œVEIS ===\nData da exportaÃ§Ã£o: ${new Date().toLocaleString('pt-BR')}\n\n`;
    ...
    relatorioTexto += `--- 2. HISTÃƒâ€œRICO DE VENDAS RECENTES ---\n`;
    ...
    relatorioTexto += `\n--- 3. MOVIMENTAÃƒâ€¡Ãƒâ€¢ES FINANCEIRAS / CAIXA ---\n`;
...
function renderVendas() {
    ...
    filtrados = filtrados.filter(v => v.tipo !== 'ORÃƒâ€¡AMENTO');
    
    if (tipoFiltro === 'VENDAS') filtrados = filtrados.filter(v => v.tipo === 'VENDA' || !v.tipo);
    if (tipoFiltro === 'SERVIÃƒâ€¡OS') filtrados = filtrados.filter(v => v.tipo === 'SERVIÃƒâ€¡O');
    ...
    document.getElementById('tabela-vendas-body').innerHTML = filtrados.map(v => {
        try {
            ...
            const badgeTipo = v.tipo === 'SERVIÃƒâ€¡O' ? `<span class="bg-purple-100 dark:bg-purple-900/40 text-purple-800 dark:text-purple-400 px-2 py-0.5 rounded text-[10px] font-bold inline-block mb-1 whitespace-nowrap">SERVIÃƒâ€¡O</span><br>` : `<span class="bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-400 px-2 py-0.5 rounded text-[10px] font-bold inline-block mb-1 whitespace-nowrap">VENDA</span><br>`;
...
// NOVO: FunÃ§Ãµes auxiliares para VÃƒ-nculo de XML
