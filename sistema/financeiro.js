...
function calcularAcrescimos() {
    ...
    const f = db.financeiro?.find(x => String(x.id) === String(id));
    if (!f) return 0;
    const ac = parseInputMoney(document.getElementById('baixa-acrescimo')?.value) || 0;
    const de = parseInputMoney(document.getElementById('baixa-desconto')?.value) || 0;
    ...
    try {
        ...
        if (db.caixa) {
            ...
            if (typeof window.FCCache !== 'undefined' && typeof window.FCCache.set === 'function') {
                ...
                window.FCCache.set('caixa_caixa_atual', db.caixa);
                window.FCCache.set('caixa_' + caixaDocId, db.caixa);
...
// ==========================================
// RENEGOCIAÇÃO AVANÇADA DE TÍTULOS FINANCEIROS
// Suporte completo a Contas a Pagar (Dívida Própria) & Contas a Receber (Clientes)
// ==========================================
...
function abrirModalRenegociacao(id) {
    ...
    // Data padrão da 1ª parcela: hoje + 30 dias
    const dIni = new Date();
...
function gerarPreviewParcelasRenegociacao() {
    ...
    // Divisão base do valor por parcela
...
async function confirmarRenegociacaoAvancada() {
    ...
    for (let i = 0; i < valoresInp.length; i++) {
        ...
        if (!dt) {
            return showToast(`Preencha a data de vencimento da parcela ${i + 1}.`, 'error');
        }
        if (val <= 0) {
            return showToast(`O valor da parcela ${i + 1} deve ser maior que zero.`, 'error');
        }
    ...
    if (dif > 0.05) {
        return showToast(`A soma das parcelas (R$ ${somaParcelas.toFixed(2)}) difere do saldo a parcelar (R$ ${saldoAParcelar.toFixed(2)}). Corrija antes de salvar.`, 'warning');
    }
    ...
    try {
        ...
        // 1. Atualizar Título Original como RENEGOCIADO
        ...
        // Atualiza a visualização da aba ativa
...
window.abrirModalNegociacao = abrirModalRenegociacao;
...
function recalcularTotalDeclarado() {
    const d = parseInputMoney(document.getElementById('fc-dinheiro').value) || 0;
    const deb = parseInputMoney(document.getElementById('fc-debito').value) || 0;
    const cred = parseInputMoney(document.getElementById('fc-credito').value) || 0;
    const pix = parseInputMoney(document.getElementById('fc-pix').value) || 0;
    const outros = parseInputMoney(document.getElementById('fc-outros').value) || 0;
...
async function confirmarTransferenciaCaixa() {
    const valor = parseInputMoney(document.getElementById('transf-valor').value) || 0;
...
async function confirmarTransferenciaFin() {
    ...
    const valor = parseInputMoney(document.getElementById('transf-fin-valor')?.value) || 0;
...
function parseOFXContent(rawText) {
    ...
    while ((matchBloco = regexBloco.exec(text)) !== null) {
        ...
        const valorNum = parseInputMoney(trnAmtRaw.replace(',', '.')) || 0;
...
async function conciliarLoteOFX() {
    ...
    indices.forEach(idx => {
        ...
        if (item.matchDoc && (item.statusMatch === 'MATCH_EXATO' || item.statusMatch === 'MATCH_PROVAVEL')) {
            const finRef = window.getEmpresaRef().collection('financeiro').doc(String(item.matchDoc.id));
            batch.update(finRef, {
                status: 'PAGO',
...
async function confirmarConciliacaoModalOFX() {
    ...
    const valorPago = parseInputMoney(document.getElementById('ofx-modal-valor-pago')?.value) || item.valor || 0;
    ...
    if (isNovo) {
        ...
        const finRef = window.getEmpresaRef().collection('financeiro').doc(String(idTituloVinculado));
        batch.update(finRef, {
            pessoa: pessoa,
            categoria: categoria,
            centroCusto: centroCusto,
...
window.verDetalhesVenda = async function(id) {
    ...
    if (!id || id === 'undefined' || id === 'null') {
        if (typeof showToast === 'function') showToast('Identificador de venda invlido.', 'warning');
        else alert('Identificador de venda invlido.');
        return;
    }

    let modal = document.getElementById('modal-detalhes-venda');
    if (!modal) {
        console.error('[VerVenda] Elemento modal-detalhes-venda no encontrado no DOM!');
        return;
    }

    // Garante que o modal esteja como filho direto do body para no ser afetado por overflow/transform
    ...
    if (!v && typeof firestore !== 'undefined') {
        try {
            ...
            const snap = await window.getEmpresaRef().collection('vendas').doc(String(id)).get();
            if (snap.exists) {
                v = { id: snap.id, ...snap.data() };
            } else {
                const numVal = Number(id);
                if (!isNaN(numVal)) {
                    const q = await window.getEmpresaRef().collection('vendas').where('numeroPedido', '==', numVal).limit(1).get();
                    if (!q.empty) {
    ...
    if (!v) {
        console.warn('[VerVenda] Venda no encontrada:', id);
        if (typeof showToast === 'function') showToast('Venda #' + id + ' no encontrada.', 'warning');
        else alert('Venda #' + id + ' no encontrada.');
        return;
    }
    ...
    if (subtitleEl) {
        subtitleEl.innerText = isGestao ? 'Viso Gerencial de Custos e Lucros' : 'Viso Detalhada';
    }
    ...
    if (elPag) elPag.innerText = tipoTexto === 'ORAMENTO' ? 'Oramento' : (v.pag || '-');

    let osInfoHtml = '';
    if (tipoTexto === 'SERVIO' && v.servicoDetalhes) {
        let galeriaHtml = '';
        if (v.servicoDetalhes.fotos && v.servicoDetalhes.fotos.length > 0) {
            galeriaHtml = `<p class="mt-2"><strong>Fotos de Referncia:</strong></p><div class="flex gap-2 flex-wrap mt-1">${v.servicoDetalhes.fotos.map(f => `<img src="${f}" class="h-20 rounded border border-purple-300 shadow-sm hover:opacity-80 transition" title="Foto">`).join('')}</div>`;
        } else if (v.servicoDetalhes.foto) {
            galeriaHtml = `<p class="mt-2"><strong>Foto de Referncia:</strong></p><img src="${v.servicoDetalhes.foto}" class="mt-1 h-24 rounded border border-purple-300 shadow-sm hover:opacity-80 transition" title="Foto">`;
        }
        osInfoHtml = `
            <div class="mt-4 bg-purple-50 dark:bg-purple-900/20 p-3 md:p-4 rounded-lg border border-purple-200 dark:border-purple-800/50 text-xs md:text-sm text-purple-900 dark:text-purple-200">
                <h4 class="font-bold mb-2 uppercase text-purple-700 dark:text-purple-300 border-b border-purple-200 dark:border-purple-800/50 pb-2"><i class="fa-solid fa-clipboard-list"></i> Ficha da Ordem de Servio</h4>
    ...
    if (elObs) {
        elObs.innerHTML = (v.obs ? v.obs : '<span class="text-slate-400 italic">Nenhuma observao geral vinculada a esta venda.</span>') + osInfoHtml;
    }
